...
function gtx {
    param (
     ...
     switch ($path) {
    #    "l" { Set-Location "//wsl.localhost/Ubuntu/home/vzze" }
        "wc" { Set-Location "C:" }
        "wd" { Set-Location "D:" }
        "we" { Set-Location "E:" }
        "pe" { Set-Location "D:/CB/Personal" }
        "wo" { Set-Location "D:/CB/Work" }
        "rr" { start shell:RecycleBinFolder }
        "de" { Set-Location "~/Desktop" }
        "do" { Set-Location "~/Downloads" }
